...
### 项目构成
- `Host` Wind2服务主机,相当于被控端,对于高阶用户来说只需要这个就可以了
- `AesDecryptor` 被控和控制器间交流的数据的解密工具,开发用
- `AesEncryptor` 被控和控制器间交流的数据的加密工具,开发用
- `ConsoleApp?` 随便写点测试用
- `Controller` Wind2控制器(Windows版),可用于图形界面控制服务主机,适合普通用户使用